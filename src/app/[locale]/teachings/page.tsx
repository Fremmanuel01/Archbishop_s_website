    ...
    <>
      ...
      <section className="py-16 bg-navy">
        <div className="section-container">
          ...
          <SectionReveal>
            {teachings?.length === 0 && (
              <p className="text-center text-cream/40 py-20">No teachings added yet. Add them in the CMS Studio.</p>
            )}
                ...
                return (
                  <div key={item._id} data-reveal className="card-surface group overflow-hidden">
                    <Link href={`/${locale}/teachings/${item.slug?.current}`}>
                      <div className="aspect-video overflow-hidden bg-navy-light relative">
                        {imgUrl ? (
                        ...
                        <span className="absolute top-3 left-3 text-[9px] uppercase tracking-widest text-gold bg-navy/80 px-2 py-1">
                          {categoryLabel(item.category, locale)}
                        </span>
                      </div>
                      <div className="p-5">
                        <p className="text-[10px] text-cream/40 mb-2">{formatDate(item.publishedAt, locale)}</p>
                        <h3 className="font-heading text-lg text-cream group-hover:text-gold transition-colors line-clamp-2">
                          {item.title}
                        </h3>
                        {item.excerpt && <p className="text-xs text-cream/50 mt-2 line-clamp-2">{item.excerpt}</p>}
                      </div>
                    </Link>
                    {item.pdf && (
